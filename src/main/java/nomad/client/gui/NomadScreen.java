...
public final class NomadScreen extends Screen {
    ...
    private void initAccounts(int y) {
        ...
        offlineNameBox.setHint(Component.literal("offline name").withStyle(EditBox.DEFAULT_HINT_STYLE));
    ...
    public void extractRenderState(GuiGraphicsExtractor ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.centeredText(font, "Nomad", width / 2, 10, 0xFFFFFFFF);
        ...
        if (tab == Tab.PROXIES) {
            ...
            ctx.centeredText(font, activeLine, width / 2, height - 34, 0xFF55FFFF);
        }
        if (!status.isEmpty())
            ctx.centeredText(font, status, width / 2, height - 20, 0xFFFFD966);
    }
